...
const FILE_TEMPLATES: Omit<DriveItem, "id">[] = [
  ...
  { name: "Hero Banner.png",            type: "image", modified: "Aug 25, 2026", size: "3.1 MB",  owner: people[6].owner, ownerInitials: people[6].ownerInitials, ownerColor: people[6].ownerColor, ownerAvatar: people[6].ownerAvatar, location: "My Drive" },
  { name: "Campaign Video.mp4",         type: "video", modified: "Aug 20, 2026", size: "210 MB",  owner: people[7].owner, ownerInitials: people[7].ownerInitials, ownerColor: people[7].ownerColor, ownerAvatar: people[7].ownerAvatar, location: "My Drive" },
...
export const allFolderIds = folderTree.map(f => f.id).filter(id => id !== "td-root");

export const myDriveFolders: DriveItem[] = folderTree
  .filter(f => f.parentId === null && f.id !== "td-root")
  .map((f, i) => ({
    id: f.id,
    name: f.name,
    type: "folder" as const,
