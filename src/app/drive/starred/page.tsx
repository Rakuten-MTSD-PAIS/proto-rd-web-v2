  ...
  return (
    <div className="flex min-h-full flex-col">
      <div className="border-b border-border-subtle px-4 pb-4 pt-5 sm:px-6">
        <PageToolbar breadcrumbs={[{ label: "Starred" }]} showActions={false} selectionCount={selectedItems.length} />
      </div>
      <section className="flex min-w-0 flex-1">
        ...
        {folderInfoOpen && (
          <RightSidePanel
            items={selectedItems}
            folderInfo={starredItems.find((item) => item.type === "folder") ?? starredItems[0]}
...
function SelectionActionToolbar({ selectedItems }: { selectedItems: DriveItem[] }) {
  ...
  const primaryActions = [
    ...
    { label: "Remove from starred", icon: Star, hideBelow: "xl", onClick: undefined as (() => void) | undefined },
        ...
        {moreOpen && (
          ...
          >
            {[
              ...
              { label: "Remove from starred", icon: Star, hideAbove: "xl", onClick: undefined as (() => void) | undefined },
...
function StarredListHeader({
  ...
  return (
    <div
      className={`mt-3 -mx-4 grid h-[42px] items-center border-b border-[#F2F2F7] bg-white text-left text-[14px] font-normal leading-[20px] text-muted-foreground shadow-[0_1px_0_#E5E5EA] sm:-mx-6 ${STARRED_COLUMNS}`}
    >
      <span className="px-3">
        <Checkbox
          checked={allSelected}
    ...
    <div className="w-full overflow-hidden">
      <table className="block w-full border-collapse">
        <thead className="sr-only">
          <tr><th>Select</th><th>Name</th><th>Owner</th><th>Modified</th><th>Size</th><th>Location</th></tr>
        </thead>
