    ...
    <div className="flex flex-col gap-5 p-4 sm:p-6">
      ...
      <section className="mx-auto w-full max-w-[760px] rounded-[12px] border border-[#E5E5EA] bg-white p-5 sm:p-6">
        <div className="mb-5">
          ...
          <p className="mt-1 text-[14px] leading-[20px] text-foreground/50">Share files securely with anyone by email.</p>
        </div>
        ...
        >
          ...
          <span className="mt-1 text-[13px] text-foreground/50">You can select multiple files to send.</span>
        </button>
          ...
          <ul className="mt-4 divide-y divide-[#E5E5EA] rounded-[8px] border border-[#E5E5EA]" aria-label="Selected files">
            {files.map((file, index) => (
              <li key={`${file.name}-${index}`} className="flex items-center gap-3 px-3 py-2.5">
                ...
                <span className="text-[12px] text-foreground/50">{Math.ceil(file.size / 1024)} KB</span>
                <button type="button" onClick={() => setFiles((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="flex size-7 items-center justify-center rounded-[6px] text-foreground/50 hover:bg-[#F2F2F7]" aria-label={`Remove ${file.name}`}>
                  <X size={16} />
                </button>
              </li>
            ))}
        ...
        <input id="recipient" type="email" value={recipient} onChange={(event) => { setRecipient(event.target.value); setSent(false); }} placeholder="name@example.com" className="mt-2 h-10 w-full rounded-[8px] border border-[#E5E5EA] px-3 text-[14px] text-[#18181A] outline-none placeholder:text-foreground/50 focus:border-[#002896]" />
        <div className="mt-5 flex items-center justify-between gap-3">
          <p className="text-[13px] text-foreground/50">{files.length ? `${files.length} file${files.length > 1 ? "s" : ""} ready to send` : "Choose files to continue"}</p>
