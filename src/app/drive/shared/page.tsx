...
export default function SharedPage() {
  ...
  function toggleAllSelection(checked: boolean) {
    ...
    setSelectedItems((current) =>
      checked
        ? [...new Map([...current, ...allVisible].map((item) => [item.id, item])).values()]
  ...
  return (
    <div className="flex min-h-full flex-col">
      <div className="border-b border-border-subtle px-4 pb-4 pt-5 sm:px-6">
        <PageToolbar breadcrumbs={[{ label: "Shared with Me" }]} showActions={false} selectionCount={selectedItems.length} />
      </div>
      <section className="flex min-w-0 flex-1">
        ...
        {folderInfoOpen && (
          <RightSidePanel
            items={selectedItems}
            folderInfo={sharedItems.find((item) => item.type === "folder") ?? sharedItems[0]}
    ...
    <div className="w-full overflow-hidden">
      <table className="block w-full border-collapse">
        <thead className="sr-only">
          <tr><th>Select</th><th>Name</th><th>Owner</th><th>Modified</th><th>Size</th></tr>
        </thead>
