...
export default function MyDrivePage() {
  ...
  function toggleItemSelection(item: DriveItem) {
    setSelectedItems((current) => current.some((selected) => selected.id === item.id)
...
function MyDriveListHeader({ items, selectedItems, onToggleAll }: { items: DriveItem[]; selectedItems: DriveItem[]; onToggleAll: (checked: boolean) => void }) {
  ...
  return <div className={`mt-3 -mx-4 grid h-[42px] items-center border-b border-[#F2F2F7] bg-white text-left text-[14px] font-normal leading-[20px] text-muted-foreground shadow-[0_1px_0_#E5E5EA] sm:-mx-6 ${MY_DRIVE_COLUMNS}`}>
